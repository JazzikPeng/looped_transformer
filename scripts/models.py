...
def build_model(conf):
    ...
    elif conf.family == 'gpt2_looped_with_emb_layer':
        model = TransformerModelLoopedWithEmbLayer(
            n_dims=conf.n_dims,
            n_positions=conf.n_positions,
            n_embd=conf.n_embd,
            n_layer=conf.n_layer,
...
class TransformerModelWithEmbLayer(nn.Module):
    ...
    def _combine(self, xs_b, ys_b):
        """
        :param xs_b: shape [B, n-p-1, d_in]
        :param ys_b: shape [B, p+1]
        """
        B, n, d = xs_b.shape
        device = xs_b.device
        
        ys_b_zeros = torch.zeros_like(ys_b)  # [B, p+1, 1]
        zs = torch.cat((xs_b, ys_b_zeros), dim=1) 
        return zs
    ...
    def forward(self, xs, ys, add_inputs_embeds=False):
        ...
        zs = self.emb_layer(zs).squeeze(2)  # [B, :, 1, d] -> [B, :, d]
...
class TransformerModelLoopedWithEmbLayer(TransformerModelWithEmbLayer):
    def __init__(
            self, n_dims, n_positions, n_embd=128, n_layer=12, n_head=4, loop_func='z=f(x+z)', pred_type='classification'):
        
        super(TransformerModelLoopedWithEmbLayer, self).__init__(
            n_dims, n_positions, n_embd, n_layer, n_head, pred_type)
        self.loop_func = loop_func
    
    def f(self, output, embeds):
        if self.loop_func == 'z=f(x+z)':
            f_output = self._backbone(inputs_embeds=output + embeds) # also of shape [B, 2n, d]
    ...
    def forward(self, xs, ys, n_loop_start, n_loops):
        ...
        pred_seq_len = ys.shape[1]  # [B, p+1]
        # Apply embedding layer to xs and ys 
        xs = self.emb_layer(xs).squeeze(2) #  [B, :, d] # length of train sequence
        ys = self.emb_layer(ys).squeeze(2) # [B, :, d] # length of prediction sequence
        zs = torch.cat((xs, ys), dim=1) # [B, seq_len + 5 + p+1, d]
        embeds = self._read_in(zs) 
        
        # Initialize loop_func
